$(function () {
  ...
  // подія кліку для кнопки 'Start game!' 
  ...
  // подія кліку для кнопки 'New game' 
  $('.newGame').on('click', function () {
    $('.first-block>div').css('opacity', 1) // повертаем прозорість якщо елемент було заблоковано
    $('.first-block>div').css('background', '') // Очистка фону якщо був заданий
    $(function () {
      var $divs = $('.img');
      var arr = [];
      $divs.each(function () {
      ...
      arr.sort(function () {
        return Math.random() - 0.5; //рандомне розміщення частин картинки
      });
  ...
  // перевірка чи добре складений пазл
  ...
  $('#check').on('click', function () {
    ...
    if (arr.length == numbers.length && checked == 0) {
      for (let i = 0; i < arr.length; i++) {
        //перевірка по внутрішньому тексту елемента
